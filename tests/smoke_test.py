...
async def main() -> None:
    ...
    # --- renew ---
    async with async_session() as s:
        svc = (await s.execute(select(Service))).scalars().first()
    bal_r = user.balance
    with patch("app.services.XUIPanel") as MockPanel:
        inst = AsyncMock()
        inst.renew_client = AsyncMock(return_value={"success": True})
        MockPanel.return_value.__aenter__.return_value = inst
        await feed_cb(f"svc_renew:{svc.id}")
        calls = await feed_cb(f"dorenew:{svc.id}:1")
    async with async_session() as s:
        user = (await s.execute(select(User).where(User.id == 222))).scalar_one()
        svc2 = (await s.execute(select(Service).where(Service.id == svc.id))).scalar_one()
    ...
    # admin approves with 25%
    await feed(f"/agentok {req.id} 25", uid=111)
    async with async_session() as s:
        user = (await s.execute(select(User).where(User.id == 222))).scalar_one()
    ...
    await feed_cb(f"vpay:zarinpal:{pay.id}")
    async with async_session() as s:
        user = (await s.execute(select(User).where(User.id == 222))).scalar_one()
        pay = (await s.execute(select(Payment).where(Payment.id == pay.id))).scalar_one()
    ...
    async with async_session() as s:
        ...
        if svcs:
            svcs[0].expire_at = _dt.utcnow() + _td(days=1); svcs[0].status = "active"
        if len(svcs) > 1:
            svcs[1].expire_at = _dt.utcnow() - _td(days=1); svcs[1].status = "active"
        await s.commit()
    try:
    ...
    await feed_cb(f"svc_transfer:{tsvc.id}")
    await feed("333")    # transfer to user 333
    async with async_session() as s:
        moved = (await s.execute(select(Service).where(Service.id == tsvc.id))).scalar_one()
